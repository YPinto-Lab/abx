...
def generate_pdf(merged, summary, summary_rel, pdf_path: str = None, phase_order=PHASE_ORDER):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for subj in subjects:
            ...
            subj_summary_abs = subj_grp.groupby("bucket", observed=True).agg(mean_vir=("pct_vir", "mean"), mean_cel=("pct_cel", "mean"), mean_num_virus_species=("num_virus_species", "mean")).reset_index()
            ...
            if "mean_num_virus_species" in subj_summary_abs.columns and subj_summary_abs["mean_num_virus_species"].notna().any():
                ...
                axes_s_abs[2].set_xticklabels(subj_summary_abs["bucket"], rotation=45)
            else:
